    ...
    <div class="col-12">
      <div class="card card-info">
        <div class="card-header">
          <h3 class="card-title">All users of <strong>{{ config('app.name', 'Laravel') }}</strong></h3>
        </div>
        <!-- /.card-header -->
        <div class="card-body">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        @if($value->status == '0')
                         <div class="d-flex">
                              <form action="{{ url('/new/user/active') }}" method="POST">
                                  {{ csrf_field() }}
                              ...
                              <form action="{{ url('/new/user/deny') }}" method="POST">
                                  {{ csrf_field() }}
                                  <input type="hidden" name="user_id" value="{{ $value->id }}">
                      ...
                      @elseif( $value->status == '1')
                          <div class="d-flex">
                              <form action="{{ url('/new/user/pause') }}" method="POST" class="mr-1">
                                  {{ csrf_field() }}
                                  <input type="hidden" name="user_id" value="{{ $value->id }}">
                      ...
                      @elseif( $value->status == '2')
                          <div class="d-flex">
                              <form action="{{ url('/new/user/active') }}" method="POST"  class="mr-1">
                                  {{ csrf_field() }}
                                  <input type="hidden" name="user_id" value="{{ $value->id }}">
                                    <button type="submit" class="btn btn-success btn-sm swalDefaultSuccess mr-2">
                                        <i class="fas fa-check-circle"></i>
